...
import type { HeroWeeklyProgress, ParentDashboardSummary } from './useHomeHeroData';
    ...
    <SafeAreaView style={styles.safe}>
      ...
      {activeRole === 'child' ? (
        ...
        <>
          ...
          {parentTab === 'home' && data.backendEnabled && <ParentHome dashboard={data.parentDashboard} pendingQuestCount={data.pendingQuests.length} pendingRewardCount={data.pendingRewards.length} onEnrollHero={() => setEnrollingHero(true)} onOpenReview={() => setParentTab('approvals')} />}
...
function ParentHome({ dashboard, pendingQuestCount, pendingRewardCount, onEnrollHero, onOpenReview }: { dashboard: ParentDashboardSummary; pendingQuestCount: number; pendingRewardCount: number; onEnrollHero: () => void; onOpenReview: () => void }) {
  const pending = pendingQuestCount + pendingRewardCount;
  const summary = dashboard.heroNames.length === 0 ? 'Enroll your first Hero to begin.' : dashboard.heroNames.length === 1 ? `${dashboard.heroNames[0]}’s quest summary` : `${dashboard.heroNames.length} Heroes in your party`;
  return <ScrollView contentContainerStyle={styles.content}><Text style={styles.pageTitle}>{greeting()}, {dashboard.leaderName}</Text><Text style={styles.pageLead}>{summary}</Text>
    <Panel style={styles.enrollmentCard}><View style={styles.enrollmentCopy}><Text style={styles.cardTitle}>{dashboard.heroNames.length === 0 ? 'Enroll your first Hero' : 'Add another Hero'}</Text><Text style={styles.muted}>Create a child-safe username and six-digit PIN. No Hero email address is needed.</Text></View><Pressable accessibilityRole="button" onPress={onEnrollHero} style={styles.enrollButton}><Ionicons name="person-add-outline" size={18} color={colors.white} /><Text style={styles.enrollButtonText}>Enroll Hero</Text></Pressable></Panel>
