...
import { Alert, Modal, Pressable, SafeAreaView, ScrollView, StyleSheet, Text, View, useWindowDimensions } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { LinearGradient } from 'expo-linear-gradient';
import { Panel, Pill, ProgressBar, QuestCard } from './components';
import { initialQuests, rewards, week } from './data';
...
export function HomeHeroApp() {
  ...
  const [xp, setXp] = useState(324);

  const complete = (quest: Quest) => {
    if (quest.kind === 'timer') return setTimerQuest(quest);
    if (quest.kind === 'guild') {
      setQuests(list => list.map(q => q.id === quest.id ? { ...q, status: 'pending_approval' } : q));
  ...
  const award = (quest: Quest) => {
    setQuests(list => list.map(q => q.id === quest.id ? { ...q, status: 'rewarded' } : q));
    setStars(value => value + quest.stars);
    setXp(value => value + quest.xp);
    setTimerQuest(null);
  };

  const approveGuild = () => {
    const quest = quests.find(q => q.status === 'pending_approval');
    if (!quest) return;
    award(quest);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      {role === 'child' ? (
        <>
          ...
          {childTab === 'store' && <StarStore stars={stars} onRedeem={redeem} />}
        ...
        <>
          {parentTab === 'home' && <ParentHome quests={quests} approveGuild={approveGuild} />}
          {parentTab === 'quests' && <QuestAdmin quests={quests} onSave={saveQuest} onRemove={removeQuest} />}
          {parentTab === 'approvals' && <Approvals quests={quests} approve={approveGuild} />}
      ...
      <TimerModal quest={timerQuest} onClose={() => setTimerQuest(null)} onFinish={() => timerQuest && award(timerQuest)} />
    </SafeAreaView>
  );
}
...
function StarStore({ stars, onRedeem }: { stars: number; onRedeem: (cost: number, title: string) => void }) {
  return <ScrollView contentContainerStyle={styles.content}><View style={styles.sectionHeader}><View><Text style={styles.pageTitle}>Star Store</Text><Text style={styles.pageLead}>Real rewards for heroic habits</Text></View><View style={styles.starBalance}><Text style={styles.starBalanceText}>⭐ {stars}</Text></View></View>
    {rewards.map(reward => <Pressable key={reward.id} onPress={() => onRedeem(reward.cost, reward.title)} style={styles.storeCard}><Text style={styles.storeEmoji}>{reward.emoji}</Text><View style={{ flex: 1 }}><Text style={styles.questTitle}>{reward.title}</Text><Text style={styles.muted}>{reward.subtitle}</Text></View><View style={styles.cost}><Text style={styles.costText}>{reward.cost} ⭐</Text></View></Pressable>)}
...
function ParentHome({ quests, approveGuild }: { quests: Quest[]; approveGuild: () => void }) {
  const done = quests.filter(q => q.status === 'rewarded').length;
  const pending = quests.filter(q => q.status === 'pending_approval').length;
...
function Approvals({ quests, approve }: { quests: Quest[]; approve: () => void }) {
...
const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: colors.cream }, content: { padding: 18, paddingBottom: 110, gap: 16 },
  ...
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 10 }, cardTitle: { color: colors.ink, fontSize: 16, fontWeight: '800' }, questTitle: { color: colors.ink, fontSize: 15, fontWeight: '800' }, muted: { color: colors.muted, fontSize: 12, lineHeight: 18 }, sectionTitle: { fontSize: 21, color: colors.navy, fontWeight: '900' }, pageTitle: { fontSize: 27, color: colors.navy, fontWeight: '900' }, pageLead: { fontSize: 13, color: colors.muted, marginTop: -12 }, tipTitle: { color: '#7A5700', fontWeight: '900', marginBottom: 4 },
