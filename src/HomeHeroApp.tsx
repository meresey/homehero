...
function BottomNav({ value, onChange, items }: { value: string; onChange: (v: string) => void; items: string[][] }) {
  ...
  return <View style={styles.nav}>{items.map(([id, icon, label]) => <Pressable accessibilityRole="button" accessibilityLabel={label} key={id} onPress={() => onChange(id)} style={styles.navItem}><Ionicons name={icon as never} size={23} color={value === id ? colors.green : colors.muted} />{!iconOnly && <Text style={[styles.navText, value === id && styles.navActive]}>{label}</Text>}</Pressable>)}</View>;
}
