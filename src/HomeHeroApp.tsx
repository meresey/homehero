...
export function HomeHeroApp() {
  ...
  const parentReviewCount = data.backendEnabled
    ? data.pendingQuests.length + data.pendingRewards.length
    : householdData.state.guildApprovals.filter(item => item.status === 'pending').length + householdData.state.rewardRequests.filter(item => item.status === 'pending').length;
...
function ParentHome({ householdName, householdId, currentUserId, dashboard, pendingQuests, pendingRewards, runningTimers, onEnrollHero, onManageHero, onOpenReview, onOpenQuests, onOpenRewards }: { householdName: string; householdId: string; currentUserId: string; dashboard: ParentDashboardSummary; pendingQuests: Quest[]; pendingRewards: RewardRedemption[]; runningTimers: Quest[]; onEnrollHero: () => void; onManageHero: (hero: ManagedHeroAccount) => void; onOpenReview: () => void; onOpenQuests: () => void; onOpenRewards: () => void }) {
  ...
  return <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <LinearGradient colors={[colors.navy, '#284A7D']} style={[styles.parentWelcome, mobile && styles.parentWelcomeMobile]}>
      ...
      <Pressable accessibilityRole="button" onPress={onEnrollHero} style={styles.parentWelcomeButton}><Ionicons name="person-add-outline" size={17} color={colors.navy} /><Text style={styles.parentWelcomeButtonText}>{mobile ? 'Enroll' : 'Enroll Hero'}</Text></Pressable>
    </LinearGradient>

    <View style={styles.dashboardSectionHeader}><View><Text style={styles.sectionTitle}>Needs your attention</Text>{!mobile && <Text style={styles.muted}>Decisions and time-sensitive activity.</Text>}</View>{pendingCount > 0 && <Pill tone="purple">{pendingCount} TO REVIEW</Pill>}</View>
    ...
    {mobile && <Pressable accessibilityRole="button" onPress={() => setShowHouseholdTools(value => !value)} style={styles.householdToolsButton}><Ionicons name="settings-outline" size={18} color={colors.navy} /><Text style={styles.householdToolsText}>Household access</Text><Ionicons name={showHouseholdTools ? 'chevron-up' : 'chevron-down'} size={18} color={colors.muted} /></Pressable>}
    {(!mobile || showHouseholdTools) && <>{dashboard.managedHeroes.length > 0 && <Panel><View style={styles.enrollmentCard}><View style={styles.enrollmentCopy}><Text style={styles.cardTitle}>Hero access</Text><Text style={styles.muted}>Manage the child-safe usernames and PINs your Heroes use to sign in.</Text></View></View><View style={styles.heroLoginList}>{dashboard.managedHeroes.map(hero => <View key={hero.userId} style={styles.heroLoginRow}><View style={styles.heroLoginIcon}><Ionicons name="person-outline" size={20} color={colors.navy} /></View><View style={styles.heroLoginCopy}><Text style={styles.heroLoginName}>{hero.displayName}</Text><Text style={styles.heroLoginUsername}>@{hero.username}</Text></View><Pressable accessibilityRole="button" accessibilityLabel={`Manage ${hero.displayName}'s login`} onPress={() => onManageHero(hero)} style={styles.manageLoginButton}><Ionicons name="key-outline" size={16} color={colors.navy} />{!mobile && <Text style={styles.manageLoginText}>Manage login</Text>}</Pressable></View>)}</View></Panel>}<PartyLeaders householdId={householdId} currentUserId={currentUserId} /></>}
  </ScrollView>;
}

function HeroOverview({ today, weekly, pendingQuests, pendingRewards, runningTimers, onEnrollHero, onOpenReview, compact = false }: { today: HeroTodayProgress[]; weekly: HeroWeeklyProgress[]; pendingQuests: Quest[]; pendingRewards: RewardRedemption[]; runningTimers: Quest[]; onEnrollHero: () => void; onOpenReview: () => void; compact?: boolean }) {
...
function BottomNav({ value, onChange, items }: { value: string; onChange: (v: string) => void; items: NavItem[] }) {
  const iconOnly = useWindowDimensions().width < 340;
  return <View style={styles.nav}>{items.map(([id, icon, label, badge = 0]) => <Pressable accessibilityRole="button" accessibilityLabel={badge > 0 ? `${label}, ${badge} awaiting review` : label} key={id} onPress={() => onChange(id)} style={styles.navItem}><View><Ionicons name={icon as never} size={23} color={value === id ? colors.green : colors.muted} />{badge > 0 && <View style={styles.navBadge}><Text style={styles.navBadgeText}>{badge > 99 ? '99+' : badge}</Text></View>}</View>{!iconOnly && <Text style={[styles.navText, value === id && styles.navActive]}>{label}</Text>}</Pressable>)}</View>;
}
...
const styles = StyleSheet.create({
  ...
  roleBarMobile: { paddingHorizontal: 14, paddingVertical: 8, flexWrap: 'nowrap' },
  ...
  parentWelcomeMobile: { padding: 17, gap: 12 },
  ...
  mobileWeeklySummary: { color: '#8A6300', fontSize: 11, fontWeight: '900', marginTop: 11 },
  ...
  householdToolsButton: { minHeight: 50, paddingHorizontal: 15, borderRadius: 15, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.white, flexDirection: 'row', alignItems: 'center', gap: 10 },
  householdToolsText: { flex: 1, color: colors.navy, fontSize: 12, fontWeight: '900' },
  ...
  nav: { position: 'absolute', left: 12, right: 12, bottom: 8, backgroundColor: colors.white, borderRadius: 22, flexDirection: 'row', paddingVertical: 10, borderWidth: 1, borderColor: colors.border }, navItem: { flex: 1, alignItems: 'center', gap: 3 }, navText: { color: colors.muted, fontSize: 10, fontWeight: '700' }, navActive: { color: colors.green, fontWeight: '900' }, navBadge: { position: 'absolute', top: -8, right: -13, minWidth: 18, height: 18, paddingHorizontal: 4, borderRadius: 9, backgroundColor: colors.coral, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: colors.white }, navBadgeText: { color: colors.white, fontSize: 9, lineHeight: 12, fontWeight: '900' },
