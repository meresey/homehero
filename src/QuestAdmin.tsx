...
function QuestEditor({ value, heroes, selectedHeroIds, onSelectedHeroIds, isExisting, onClose, onSave }: { value: Quest | null | 'new'; heroes: HeroProfile[]; selectedHeroIds: string[]; onSelectedHeroIds: (ids: string[]) => void; isExisting: boolean; onClose: () => void; onSave: (quest: Quest, heroIds: string[]) => boolean | Promise<boolean> }) {
  ...
  return <Modal animationType="slide" visible onRequestClose={onClose}><SafeAreaView style={styles.modalSafe}><View style={styles.modalSurface}><View style={styles.modalHeader}><Pressable onPress={onClose}><Text style={styles.cancel}>Cancel</Text></Pressable><Text style={styles.modalTitle}>{isExisting ? 'Edit quest' : 'Add quest'}</Text><Pressable onPress={submit}><Text style={styles.save}>Save</Text></Pressable></View>
    <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
      ...
      {formError && <View style={styles.formError}><Ionicons name="alert-circle-outline" size={19} color={colors.coral} /><Text style={styles.formErrorText}>{formError}</Text></View>}
...
const styles = StyleSheet.create({
  ...
  formError: { flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 13, padding: 12, backgroundColor: '#FFF0ED', borderWidth: 1, borderColor: colors.coral }, formErrorText: { flex: 1, color: colors.coral, fontSize: 12, lineHeight: 17, fontWeight: '800' },
});
