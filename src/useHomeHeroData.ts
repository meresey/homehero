...
export function useHomeHeroData() {
  ...
  const refresh = useCallback(async (activeSession?: Session | null) => {
    ...
    setSession(current);
    if (!current) { setFamily(null); setQuests([]); setPendingQuests([]); setLoading(false); return; }
  ...
  return { backendEnabled, session, family, quests, pendingQuests, rewards, stars, xp, loading, error, refresh,
